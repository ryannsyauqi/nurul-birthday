...
type Slide = { kind: "reveal" | "letter" | "wish" | "memory" | "bangkok" | "dream" | "ending"; eyebrow: string; title: string; copy: string[]; label?: string; note?: string };
...
const slides: Slide[] = [
  ...
  { kind: "letter", eyebrow: "20 september", title: "Happy Birthday,\nMy Wife", copy: ["Nurul Rizki Yanti", "Makasih ya udah jadi istri, teman, tempat cerita, dan partner paling seru buat aku. Semoga malam ini kamu senang, dan semoga kamu selalu inget kalau aku sayang banget sama kamu."] },
  ...
  { kind: "memory", eyebrow: "our memories · 03", title: "Every trip\nwas better with you", copy: ["Tempatnya boleh beda-beda, tapi yang bikin semuanya seru tetap karena ada kamu."], note: "Foto trips kamu taruh di sini nanti" },
  ...
  { kind: "bangkok", eyebrow: "our next honeymoon", title: "Bangkok,\nhere we come", copy: ["Setelah semua cerita yang udah kita lewatin, sekarang waktunya bikin cerita baru lagi."], note: "Cuma kita berdua, jalan-jalan, makan enak, foto-foto, dan nggak mikirin apa-apa dulu." },
  { kind: "bangkok", eyebrow: "bangkok plans · 01", title: "Let’s get lost\ntogether", copy: ["Jalan pelan-pelan, cari tempat random, dan bikin cerita yang nggak ada di itinerary."], note: "Bangkok photo / map moment" },
  { kind: "bangkok", eyebrow: "bangkok plans · 02", title: "Let’s eat\neverything", copy: ["Street food, café kecil, dinner enak, dan semua makanan yang pengin kita coba."], note: "Bangkok food photo" },
  { kind: "bangkok", eyebrow: "bangkok plans · 03", title: "Just us", copy: ["Yang paling aku tunggu bukan tempatnya, tapi waktunya bareng kamu."], note: "Bangkok couple photo" },
...
function PhotoSlot({ note }: { note?: string }) {
  return <div className="photo-slot" aria-label={note ?? "Tempat foto"}><div className="tape tape-left" /><div className="tape tape-right" /><span>foto kita</span><small>{note}</small></div>;
}
    ...
    <MobileScroll className="app-screen scrapbook-screen">
      <main className="screen-content" aria-live="polite">
        ...
        <AnimatePresence mode="wait"><motion.article className="slide" key={active} initial={{ opacity: 0, y: 18, rotate: active % 2 ? -1.2 : 1.2 }} animate={{ opacity: 1, y: 0, rotate: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.42, ease: [0.22, 1, 0.36, 1] }}>
          ...
          {(slide.kind === "memory" || slide.kind === "bangkok") && <PhotoSlot note={slide.note} />}
